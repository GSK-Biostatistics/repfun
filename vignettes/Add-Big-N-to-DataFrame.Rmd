...
repfun::rs_setup(D_POP='SAFFL',
         D_POPLBL='Safety',
         D_POPDATA=get(load("../data/adsl.rda")) %>%
...
adae <- rfenv$adamdata$adae.rda() %>% filter(TRTEMFL=='Y') %>%
...
knitr::kable(
  head(addbign,20), col.names=paste(names(lbls),lbls,sep=": "), 
...
```{r cleanup}
#unlink("../docs/articles/Add-Big-N-to-DataFrame_files", recursive = TRUE)
```
